...
/*
 * Equation 5-2: 2*BRP/F_OSC = TQ
 * We use BRP = 4, F_OSC = 16MHz -> TQ = 500ns -> bit time 
 * CAN baud rate 125kHz -> 1/(125kHz*TQ) = 1/(125kHz*500ns) = 16 TQ as bit time 
 * Bit time = SyncSeg + PropSeg + PS1 + PS2
 */

/* We have chosen BRP = 4, set register to BRP - 1 */
#define BRP 3
/* Synchronization Jump Width Length is chosen as 0, i.e. length = 1*TQ */
#define SJW 0
/* Phase segment PS1 must be [1,8], set register to PS1 - 1 */
#define PS1 5
/* Phase segment PS2 must be [2,8], set register to PS2 - 1 */
#define PS2 6
/* Propagation segment must be [1,8], set register to PRSEG - 1 */
#define PRSEG 1
